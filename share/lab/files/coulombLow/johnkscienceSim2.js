...
let johnkscienceCharge = function(config){
    ...
    let colorFill = config.charge ==='+' ? config.positiveFill : config.negativeFill;
    ...
    let circle = new Konva.Circle({
        ...
        fillRadialGradientStartRadius: 4*config.radius/10,
        fillRadialGradientEndPoint: { x: 0, y: 0 },
        fillRadialGradientEndRadius: 15*config.radius/10,
        fillRadialGradientColorStops: [0,  colorFill, 1, config.stroke],
    });
    ...
    charge.setRadius = function(value){
        //Ανανεώνει την τιμή της ακτίνας
        config.radius = value;
        circle.radius(value);
        //Καταστέφει όλα τα θετικά ή αρνητικά σύμβολα
        for(let i=0; i<nuclei.length; i++){
            nuclei[i].destroy();
        }
        nuclei = [];
        //Επανασχεδιάζει τα θετικά ή τα αρνητικά σύμβολα
        a1=5;  w=5; n=1;
        for(let r=20; r<config.radius; r+=35){
            nn=a1+w*(n-1);
            for(let k=0; k<nn; k++){
        ...
        circle.fillRadialGradientStartRadius(4*config.radius/10);
        circle.fillRadialGradientEndRadius(15*config.radius/10);

    }

    charge.getRadius = function(){
